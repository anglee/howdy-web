...
function solution(n, m, posR, posC, maze) {
  const root = {
    row: 0,
    col: 0,
    step: 0,
  ...
  while (stack.length > 0) {
    ...
    if (node.row === posR - 1 && node.col === posC - 1) {
      return node.step;
    } else {
...
function processData(data) {
  ...
  console.log(solution(n, m, posR, posC, maze));
}

const solutionAAA = (input) => {
  processData(input);
};
...
0 0 1
1 1 1`;
console.log(solutionAAA(sampleInput));

export default solutionAAA;
