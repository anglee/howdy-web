...
describe('/lib Heap', () => {
  ...
  describe('delete', () => {
    it('should remove the value from heap (max)', () => {
      const array = [1,8,6,5,3,7,4];
      const heap = new Heap(array);
      heap.delete(8);
      expect(isValidHeap(heap)).to.be.true;
    ...
    it('should remove the value from heap (min)', () => {
      const array = [1,8,6,5,3,7,4];
      const heap = new Heap(array);
      heap.delete(1);
      expect(isValidHeap(heap)).to.be.true;
      expect(haveTheSameSet(heap, _.without(array, 1))).to.be.true;
    });
    it('should remove the value from heap (any)', () => {
      const array = [1,8,6,5,3,7,4];
      const heap = new Heap(array);
      heap.delete(6);
      expect(isValidHeap(heap)).to.be.true;
      expect(haveTheSameSet(heap, _.without(array, 6))).to.be.true;
    });
    it('should remove the value from heap (single)', () => {
      const array = [3];
      const heap = new Heap(array);
      heap.delete(3);
      expect(isValidHeap(heap)).to.be.true;
      expect(haveTheSameSet(heap, _.without(array, 3))).to.be.true;
    });
  });
